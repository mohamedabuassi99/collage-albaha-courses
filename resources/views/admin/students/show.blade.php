@extends('layouts.admin.app')
@section('title')
    جميع الدورات
@endsection
@section('content')
    <div class="card mb-3">
        <div class="bg-holder d-none d-lg-block bg-card"
             style="background-image:url({{asset('admin/assets/img/icons/spot-illustrations/corner-1.png')}});background-size: cover;"></div>
        <!--/.bg-holder-->
        <div class="card-body position-relative">
            <div class="row">
                <h3 class="text-center mt-3 mb-4">
                    جميع دورات الطالب {{$student->name}}
                </h3>
                @if(session('success'))
                    <div class="alert alert-success border-2 d-flex align-items-center" role="alert">
                        <div class="bg-success me-3 icon-item"><span
                                    class="fas fa-check-circle text-white fs-3"></span></div>
                        <p class="mb-0 flex-1">{{session('success')}}</p>
                        <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                @elseif(session('failed'))
                    <div class="alert alert-danger border-2 d-flex align-items-center" role="alert">
                        <div class="bg-danger me-3 icon-item"><span
                                    class="fas fa-times-circle text-white fs-3"></span></div>
                        <p class="mb-0 flex-1">{{session('failed')}}
                        </p>
                        <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                @endif
                <div class="col-12  ">
                    <form action="{{route('admin.students.index')}}" class="" method="get">
                        {{--                            @csrf--}}
                        <div class="d-flex gap-2 mb-3 justify-content-center">
                            <input type="search" name="search" class="form-control w-md-25 w-auto"
                                   placeholder="بحث بالاسم او البريد"
                                   id="">
                            <button class="btn btn-outline-primary"> بحث</button>
                        </div>
                    </form>
                </div>
                <div class="table-responsive scrollbar">
                    <table class="table table-striped  align-middle  text-center overflow-hidden">

                        <thead>
                        <tr class="btn-reveal-trigger">
                            <th scope="col">#</th>
                            <th scope="col">اسم الدورة</th>
                            <th scope="col">اسم الدورة بالانجليزي</th>

                            <th scope="col">اسم المدرس</th>
                            <th scope="col">اسم المدرس بالانجليزي</th>
                            <th scope="col">من</th>
                            <th scope="col">الى</th>
                            <th scope="col">اصدار الشهادة</th>
                            <th scope="col">عدد الساعات</th>
                            <th scope="col">عدد الايام</th>
                            <th scope="col">قيمة الدورة</th>
                            <th scope="col">الاعتماد</th>
                            <th scope="col">العمليات</th>
                        </tr>
                        </thead>
                        <tbody>
                        @forelse($student->courses as $course)
                            <tr class="btn-reveal-trigger">
                                <td>{{$loop->index+1}}</td>
                                <td>{{$course->name}}</td>
                                <td>{{$course->name_en ?? '-'}}</td>
                                <td>{{$course->instructor ?? '-'}}</td>
                                <td>{{$course->instructor_en ?? '-'}}</td>
                                <td>{{$course->from??'-'}}</td>
                                <td>{{$course->to??'-'}}</td>
                                <td>{{$course->publishdate??'-'}}</td>
                                <td>{{$course->hours??'-'}}</td>
                                <td>{{$course->days??'-'}}</td>
                                <td>{{$course->price??'-'}}</td>

                                <td>@if($course->pivot->status) <i class="fas fa-check-circle text-success"></i> @else
                                        <i
                                                class="fas fa-times-circle text-danger"></i> @endif</td>
                                <td>
                                    <a href="{{route('admin.course.student.destroy',[$student,$course])}}"
                                       class="btn btn-outline-danger">حذف</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9">لا يوجد طلاب مضافة</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
                {{--                <div class="text-center my-4">--}}
                {{--                    {{$students->links()}}--}}
                {{--                </div>--}}
            </div>
        </div>
    </div>
@endsection